...
// hold user on register page until verification
...
app.post('/api/sendGuestInvite', async (req, res) => {
  ...
  try {
    ...
    const approveUrl = `http://localhost:5001/api/updateContactStatus?guestId=${guestId}&status=1`;
    const rejectUrl = `http://localhost:5001/api/updateContactStatus?guestId=${guestId}&status=0`;
...
app.post("/api/updateContactStatus", async (req, res) => {
  // incoming: guestId, status
  // outgoing: success message or error
  console.log("Updating contact status: ", req.query);

  const { guestId, status } = req.query;
  let error = "";
  //console.log(`new status: ${status}`);
  //console.log(`guestId in question: ${guestId}`);
